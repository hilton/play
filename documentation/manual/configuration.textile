...
Cookie name used to store the language value specified by a call to @play.i18n.Lang.change(String locale)@.
...
Set the cookie to ‘HTTP only’. For example:

bc. application.session.httpOnly=true

Default: @false@

TODO: see http://www.owasp.org/index.php/HttpOnly
...
Enables session for HTTPS. For example:
...
bc. certificate.key.file=conf/host.key
...
bc. certificate.file=conf/host.cert
...
Use configuration keys that start with @cron.@ to specify named values for the <code>@On</code> and <code>@Every</code> annotations. For example:

bc. cron.noonDaily=0 0 12 * * ?
...
Custom user agent for web services requests.

TODO: value passed to @com.ning.http.client.AsyncHttpClientConfig.setUserAgent@
...
Trustmanager configuration, for use with "X509 certificates":#certificate and "keystore":#keystore configurations.

bc. trustmanager.algorithm=JKS
...
TODO. First parameter to @org.apache.commons.io.output.DeferredFileOutputStream(int, File)@
