...
Cookies will be valid for all domains ending with ‘.example.com’, e.g. @foo.example.com@ and @bar.example.com@
...
To set cache expiry to one hour (the default):

bc. http.cacheControl=3600
...
Open file from error pages. If your text editor supports opening files by URL, Play will dynamically link error pages to files. For example:

bc. play.editor=txmt://open?url=file://%s&line=%s@ for Textmate
