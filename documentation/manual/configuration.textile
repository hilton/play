...
The name of the cookie that is used to store the current language, set by @play.i18n.Lang.change(String lang)@, which you can change if you want separate language settings for separate Play applications. For example:
...
You can configure cron expressions for scheduled jobs as configuration keys that start with @cron.@ and use the key as the value of a <code>@play.jobs.On</code> annotation. For example, <code>@On(cron.noon)</code> refers to:
...
Names of additional JPA entity classes to load.
